...
const tile = (file: string) => `/city-tiles/${file}`;

const cityTileMap: Record<string, CityIllustration> = {
  'river-hill-plains': { src: tile('river.png'), label: '河流丘陵地块' },
  ...
  'industrial-hammerland': { src: tile('high-production.png'), label: '高锤工业地块' },
  'rainforest-research': { src: tile('rainforest.png'), label: '雨林科研地块' },
  'frontier-fortress': { src: tile('hills.png'), label: '丘陵军事地块' },
  'grassland-granary': { src: tile('high-food.png'), label: '高粮草原地块' },
  'lake-commercial-hub': { src: tile('balanced.png'), label: '粮锤平衡商业地块' },
  'volcano-fertile-ring': { src: tile('balanced.png'), label: '高收益平衡地块' },
  'island-harbor-city': { src: tile('coast.png'), label: '群岛港口地块' },
  'tundra-faith-outpost': { src: tile('mountains.png'), label: '山地信仰前哨' },
  'river-cultural-theater': { src: tile('river.png'), label: '河岸文化地块' },
  ...
  'iron-mountain-forge': { src: tile('high-production.png'), label: '高锤矿山地块' },
  'floodplain-dam-city': { src: tile('floodplains.png'), label: '泛滥平原水坝地块' },
  'desert-petra-site': { src: tile('desert.png'), label: '佩特拉沙漠地块' },
  'reef-science-coast': { src: tile('coast.png'), label: '礁石科研海岸' },
  'forest-lumber-town': { src: tile('forest.png'), label: '森林伐木地块' },
  'holy-mountain-valley': { src: tile('mountains.png'), label: '圣山宗教谷地' },
  'border-river-chokepoint': { src: tile('river.png'), label: '边境河流隘口' },
  'marsh-reclamation-city': { src: tile('floodplains.png'), label: '沼泽低地改良地块' },
...
export function getCityIllustration(city: CityCandidate): CityIllustration {
  ...
  if (has('粮锤平衡')) return { src: tile('balanced.png'), label: '粮锤平衡地块' };
  if (has('高产能', '工业', '矿山', '铁山', '锻造')) return { src: tile('high-production.png'), label: '高锤地块' };
  if (has('高粮', '高粮食', '粮食城', '粮仓')) return { src: tile('high-food.png'), label: '高粮地块' };
  ...
  if (has('河流', '大河', '淡水', '河岸', '河谷', '近河', '小河', '河口')) return { src: tile('river.png'), label: '河流' };
  ...
  if (has('平丘', '平原丘陵')) return { src: tile('plains-hills.png'), label: '平丘' };
  ...
  if (city.luxuryResources.length > 0 || city.strategicResources.length > 0) {
    return { src: tile('resource.png'), label: '资源地块' };
  }
